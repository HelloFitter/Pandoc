...
spnl :: PMonad m => Parser [Char] st m ()
spnl = try $ do
  skipSpaces
  optional newline
  skipSpaces
...
indentSpaces :: PMonad m => MarkdownParser m String
indentSpaces = try $ do
  tabStop <- getOption readerTabStop
  count tabStop (char ' ') <|>
    string "\t" <?> "indentation"

nonindentSpaces :: PMonad m => MarkdownParser m String
nonindentSpaces = do
  tabStop <- getOption readerTabStop
  sps <- many (char ' ')
...
skipNonindentSpaces :: PMonad m => MarkdownParser m ()
skipNonindentSpaces = do
  tabStop <- getOption readerTabStop
  atMostSpaces (tabStop - 1)

atMostSpaces :: PMonad m => Int -> MarkdownParser m ()
atMostSpaces 0 = notFollowedBy (char ' ')
atMostSpaces n = (char ' ' >> atMostSpaces (n-1)) <|> return ()

litChar :: PMonad m => MarkdownParser m Char
litChar = escapedChar'
       <|> noneOf "\n"
...
inlinesInBalancedBrackets :: PMonad m => MarkdownParser m (F Inlines)
inlinesInBalancedBrackets = charsInBalancedBrackets >>=
  parseFromString (trimInlinesF . mconcat <$> many inline)

charsInBalancedBrackets :: PMonad m => MarkdownParser m [Char]
...
titleLine :: PMonad m => MarkdownParser m (F Inlines)
titleLine = try $ do
  char '%'
  skipSpaces
...
authorsLine :: PMonad m => MarkdownParser m (F [Inlines])
authorsLine = try $ do
  char '%'
  skipSpaces
...
dateLine :: PMonad m => MarkdownParser m (F Inlines)
dateLine = try $ do
  char '%'
  skipSpaces
  trimInlinesF . mconcat <$> manyTill inline newline

titleBlock :: PMonad m => MarkdownParser m (F Inlines, F [Inlines], F Inlines)
titleBlock = pandocTitleBlock <|> mmdTitleBlock

pandocTitleBlock :: PMonad m => MarkdownParser m (F Inlines, F [Inlines], F Inlines)
...
mmdTitleBlock :: PMonad m => MarkdownParser m (F Inlines, F [Inlines], F Inlines)
...
kvPair :: PMonad m => MarkdownParser m (String, Inlines)
kvPair = try $ do
...
parseMarkdown :: PMonad m => MarkdownParser m Pandoc
parseMarkdown = do
...
referenceKey :: PMonad m => MarkdownParser m (F Blocks)
referenceKey = try $ do
  skipNonindentSpaces
  (_,raw) <- reference
  char ':'
...
referenceTitle :: PMonad m => MarkdownParser m String
...
-- an abbreviation.
abbrevKey :: PMonad m => MarkdownParser m (F Blocks)
abbrevKey = do
...
noteMarker :: PMonad m => MarkdownParser m String
noteMarker = string "[^" >> many1Till (satisfy $ not . isBlank) (char ']')

rawLine :: PMonad m => MarkdownParser m String
rawLine = try $ do
...
rawLines :: PMonad m => MarkdownParser m String
rawLines = do
  first <- anyLine
  rest <- many rawLine
  return $ unlines (first:rest)

noteBlock :: PMonad m => MarkdownParser m (F Blocks)
noteBlock = try $ do
  skipNonindentSpaces
  ref <- noteMarker
  char ':'
...
parseBlocks :: PMonad m => MarkdownParser m (F Blocks)
parseBlocks = mconcat <$> manyTill block eof

block :: PMonad m => MarkdownParser m (F Blocks)
...
header :: PMonad m => MarkdownParser m (F Blocks)
header = setextHeader <|> atxHeader <?> "header"

atxHeader :: PMonad m => MarkdownParser m (F Blocks)
atxHeader = try $ do
...
atxClosing :: PMonad m => MarkdownParser m String
atxClosing = try $ skipMany (char '#') >> blanklines

setextHeader :: PMonad m => MarkdownParser m (F Blocks)
setextHeader = try $ do
...
hrule :: PMonad m => MarkdownParser m (F Blocks)
hrule = try $ do
  skipSpaces
...
indentedLine :: PMonad m => MarkdownParser m String
indentedLine = indentSpaces >> manyTill anyChar newline >>= return . (++ "\n")

blockDelimiter :: PMonad m => (Char -> Bool)
               -> Maybe Int
...
attributes :: PMonad m => MarkdownParser m (String, [String], [(String, String)])
attributes = try $ do
  char '{'
  spnl
...
attribute :: PMonad m => MarkdownParser m (String, [String], [(String, String)])
attribute = identifierAttr <|> classAttr <|> keyValAttr

identifier :: PMonad m => MarkdownParser m String
identifier = do
  first <- letter
  rest <- many $ alphaNum <|> oneOf "-_:."
  return (first:rest)

identifierAttr :: PMonad m => MarkdownParser m (String, [a], [a1])
identifierAttr = try $ do
  char '#'
  result <- identifier
  return (result,[],[])

classAttr :: PMonad m => MarkdownParser m (String, [String], [a])
classAttr = try $ do
  char '.'
  result <- identifier
  return ("",[result],[])

keyValAttr :: PMonad m => MarkdownParser m (String, [a], [(String, String)])
keyValAttr = try $ do
  key <- identifier
  char '='
...
codeBlockFenced :: PMonad m => MarkdownParser m (F Blocks)
...
codeBlockBackticks :: PMonad m => MarkdownParser m (F Blocks)
...
codeBlockIndented :: PMonad m => MarkdownParser m (F Blocks)
...
lhsCodeBlock :: PMonad m => MarkdownParser m (F Blocks)
lhsCodeBlock = do
...
lhsCodeBlockLaTeX :: PMonad m => MarkdownParser m String
...
lhsCodeBlockBird :: PMonad m => MarkdownParser m String
lhsCodeBlockBird = lhsCodeBlockBirdWith '>'

lhsCodeBlockInverseBird :: PMonad m => MarkdownParser m String
lhsCodeBlockInverseBird = lhsCodeBlockBirdWith '<'

lhsCodeBlockBirdWith :: PMonad m => Char -> MarkdownParser m String
...
birdTrackLine :: PMonad m => Char -> MarkdownParser m String
...
emailBlockQuoteStart :: PMonad m => MarkdownParser m Char
emailBlockQuoteStart = try $ skipNonindentSpaces >> char '>' >>~ optional (char ' ')

emailBlockQuote :: PMonad m => MarkdownParser m [String]
...
blockQuote :: PMonad m => MarkdownParser m (F Blocks)
blockQuote = do
  raw <- emailBlockQuote
...
bulletListStart :: PMonad m => MarkdownParser m ()
...
anyOrderedListStart :: PMonad m => MarkdownParser m (Int, ListNumberStyle, ListNumberDelim)
...
listStart :: PMonad m => MarkdownParser m ()
listStart = bulletListStart <|> (anyOrderedListStart >> return ())

-- parse a line of a list item (start = parser for beginning of list item)
listLine :: PMonad m => MarkdownParser m String
listLine = try $ do
  notFollowedBy blankline
...
rawListItem :: PMonad m => MarkdownParser m a
...
listContinuation :: PMonad m => MarkdownParser m String
...
listContinuationLine :: PMonad m => MarkdownParser m String
...
listItem :: PMonad m => MarkdownParser m a
...
orderedList :: PMonad m => MarkdownParser m (F Blocks)
orderedList = try $ do
...
bulletList :: PMonad m => MarkdownParser m (F Blocks)
bulletList = do
...
defListMarker :: PMonad m => MarkdownParser m ()
defListMarker = do
  sps <- nonindentSpaces
  char ':' <|> char '~'
  tabStop <- getOption readerTabStop
...
definitionListItem :: PMonad m => MarkdownParser m (F (Inlines, [Blocks]))
...
defRawBlock :: PMonad m => MarkdownParser m String
defRawBlock = try $ do
  defListMarker
  firstline <- anyLine
...
definitionList :: PMonad m => MarkdownParser m (F Blocks)
definitionList = do
...
para :: PMonad m => MarkdownParser m (F Blocks)
para = try $ do
...
plain :: PMonad m => MarkdownParser m (F Blocks)
...
htmlElement :: PMonad m => MarkdownParser m String
htmlElement = strictHtmlBlock <|> liftM snd (htmlTag isBlockTag)

htmlBlock :: PMonad m => MarkdownParser m (F Blocks)
htmlBlock = do
...
htmlBlock' :: PMonad m => MarkdownParser m String
htmlBlock' = try $ do
    first <- htmlElement
...
strictHtmlBlock :: PMonad m => MarkdownParser m String
strictHtmlBlock = htmlInBalanced (not . isInlineTag)

rawVerbatimBlock :: PMonad m => MarkdownParser m String
...
rawTeXBlock :: PMonad m => MarkdownParser m (F Blocks)
rawTeXBlock = do
...
rawHtmlBlocks :: PMonad m => MarkdownParser m String
rawHtmlBlocks = do
...
simpleTableHeader :: PMonad m => Bool  -- ^ Headerless table
...
tableFooter :: PMonad m => MarkdownParser m String
tableFooter = try $ skipNonindentSpaces >> many1 (dashedLine '-') >> blanklines

-- Parse a table separator - dashed line.
tableSep :: PMonad m => MarkdownParser m Char
tableSep = try $ skipNonindentSpaces >> many1 (dashedLine '-') >> char '\n'

-- Parse a raw line and split it into chunks by indices.
rawTableLine :: PMonad m => [Int]
             -> MarkdownParser m [String]
...
tableCaption :: PMonad m => MarkdownParser m (F Inlines)
tableCaption = try $ do
...
simpleTable :: PMonad m => Bool  -- ^ Headerless table
...
multilineTable :: PMonad m => Bool -- ^ Headerless table
               -> MarkdownParser m ([Alignment], [Double], F [Blocks], F [[Blocks]])
multilineTable headless =
  tableWith (multilineTableHeader headless) multilineRow blanklines tableFooter

multilineTableHeader :: PMonad m => Bool -- ^ Headerless table
                     -> MarkdownParser m (F [Blocks], [Alignment], [Int])
...
-- ending with a footer (dashed line followed by blank line).
gridTable :: PMonad m => Bool -- ^ Headerless table
          -> MarkdownParser m ([Alignment], [Double], F [Blocks], F [[Blocks]])
gridTable headless =
...
gridPart :: PMonad m => Char -> MarkdownParser m (Int, Int)
gridPart ch = do
  dashes <- many1 (char ch)
  char '+'
  return (length dashes, length dashes + 1)

gridDashedLines :: PMonad m => Char -> MarkdownParser m [(Int,Int)]
...
gridTableSep :: PMonad m => Char -> MarkdownParser m Char
gridTableSep ch = try $ gridDashedLines ch >> return '\n'

-- | Parse header for a grid table.
gridTableHeader :: PMonad m => Bool -- ^ Headerless table
                -> MarkdownParser m (F [Blocks], [Alignment], [Int])
...
gridTableRawLine :: PMonad m => [Int] -> MarkdownParser m [String]
...
gridTableFooter :: PMonad m => MarkdownParser m [Char]
gridTableFooter = blanklines

pipeTable :: PMonad m => MarkdownParser m ([Alignment], [Double], F [Blocks], F [[Blocks]])
pipeTable = try $ do
...
sepPipe :: PMonad m => MarkdownParser m ()
sepPipe = try $ do
  char '|' <|> char '+'
  notFollowedBy blankline

-- parse a row, also returning probable alignments for org-table cells
pipeTableRow :: PMonad m => MarkdownParser m (F [Blocks])
pipeTableRow = do
  nonindentSpaces
  optional (char '|')
  let cell = mconcat <$>
...
pipeTableHeaderPart :: PMonad m => MarkdownParser m Alignment
...
table :: PMonad m => MarkdownParser m (F Blocks)
table = try $ do
...
inline :: PMonad m => MarkdownParser m (F Inlines)
...
escapedChar' :: PMonad m => MarkdownParser m Char
escapedChar' = try $ do
  char '\\'
  (guardEnabled Ext_all_symbols_escapable >> satisfy (not . isAlphaNum))
     <|> oneOf "\\`*_{}[]()>#+-.!~"

escapedChar :: PMonad m => MarkdownParser m (F Inlines)
escapedChar = do
  result <- escapedChar'
  case result of
...
ltSign :: PMonad m => MarkdownParser m (F Inlines)
ltSign = do
...
exampleRef :: PMonad m => MarkdownParser m (F Inlines)
exampleRef = try $ do
...
symbol :: PMonad m => MarkdownParser m (F Inlines)
symbol = do
  result <- noneOf "<\\\n\t "
...
code :: PMonad m => MarkdownParser m (F Inlines)
code = try $ do
  starts <- many1 (char '`')
  skipSpaces
...
math :: PMonad m => MarkdownParser m (F Inlines)
math =  (return . B.displayMath <$> (mathDisplay >>= applyMacros'))
     <|> (return . B.math <$> (mathInline >>= applyMacros'))

mathDisplay :: PMonad m => MarkdownParser m String
mathDisplay =
...
mathDisplayWith :: PMonad m => String -> String -> MarkdownParser m String
mathDisplayWith op cl = try $ do
  string op
  many1Till (noneOf "\n" <|> (newline >>~ notFollowedBy' blankline)) (try $ string cl)

mathInline :: PMonad m => MarkdownParser m String
mathInline =
...
mathInlineWith :: PMonad m => String -> String -> MarkdownParser m String
...
fours :: PMonad m => MarkdownParser m (F Inlines)
fours = try $ do
...
inlinesBetween :: (PMonad m, Show b)
               => MarkdownParser m a
               -> MarkdownParser m b
...
emph :: PMonad m => MarkdownParser m (F Inlines)
...
strong :: PMonad m => MarkdownParser m (F Inlines)
...
strikeout :: PMonad m => MarkdownParser m (F Inlines)
...
superscript :: PMonad m => MarkdownParser m (F Inlines)
superscript = fmap B.superscript <$> try (do
  guardEnabled Ext_superscript
  char '^'
  mconcat <$> many1Till (notFollowedBy spaceChar >> inline) (char '^'))

subscript :: PMonad m => MarkdownParser m (F Inlines)
subscript = fmap B.subscript <$> try (do
  guardEnabled Ext_subscript
  char '~'
  mconcat <$> many1Till (notFollowedBy spaceChar >> inline) (char '~'))

whitespace :: PMonad m => MarkdownParser m (F Inlines)
whitespace = spaceChar >> return <$> (lb <|> regsp) <?> "whitespace"
  where lb = spaceChar >> skipMany spaceChar >> option B.space (endline >> return B.linebreak)
        regsp = skipMany spaceChar >> return B.space

nonEndline :: PMonad m => MarkdownParser m Char
nonEndline = satisfy (/='\n')

str :: PMonad m => MarkdownParser m (F Inlines)
str = do
...
endline :: PMonad m => MarkdownParser m (F Inlines)
endline = try $ do
  newline
  notFollowedBy blankline
...
reference :: PMonad m => MarkdownParser m (F Inlines, String)
reference = do notFollowedBy' (string "[^")   -- footnote reference
               withRaw $ trimInlinesF <$> inlinesInBalancedBrackets

-- source for a link, with optional title
source :: PMonad m => MarkdownParser m (String, String)
source =
...
linkTitle :: PMonad m => MarkdownParser m String
linkTitle = try $ do
...
link :: PMonad m => MarkdownParser m (F Inlines)
link = try $ do
  st <- getState
...
regLink :: PMonad m => (String -> String -> Inlines -> Inlines)
...
referenceLink :: PMonad m => (String -> String -> Inlines -> Inlines)
...
bareURL :: PMonad m => MarkdownParser m (F Inlines)
bareURL = try $ do
  guardEnabled Ext_autolink_urls
  (orig, src) <- uri <|> emailAddress
  return $ return $ B.link src "" (B.codeWith ("",["url"],[]) orig)

autoLink :: PMonad m => MarkdownParser m (F Inlines)
autoLink = try $ do
  char '<'
  (orig, src) <- uri <|> emailAddress
  char '>'
...
image :: PMonad m => MarkdownParser m (F Inlines)
image = try $ do
  char '!'
  (lab,raw) <- reference
  regLink B.image lab <|> referenceLink B.image (lab,raw)

note :: PMonad m => MarkdownParser m (F Inlines)
note = try $ do
...
inlineNote :: PMonad m => MarkdownParser m (F Inlines)
inlineNote = try $ do
...
rawLaTeXInline' :: PMonad m => MarkdownParser m (F Inlines)
...
rawConTeXtEnvironment :: PMonad m => MarkdownParser m String
...
inBrackets :: PMonad m => (MarkdownParser m Char) -> MarkdownParser m String
...
rawHtmlInline :: PMonad m => MarkdownParser m (F Inlines)
rawHtmlInline = do
  guardEnabled Ext_raw_html
...
cite :: PMonad m => MarkdownParser m (F Inlines)
cite = do
...
textualCite :: PMonad m => MarkdownParser m (F [Citation])
textualCite = try $ do
  (_, key) <- citeKey
...
bareloc :: PMonad m => Citation -> MarkdownParser m (F [Citation])
bareloc c = try $ do
  spnl
  char '['
  suff <- suffix
...
normalCite :: PMonad m => MarkdownParser m (F [Citation])
normalCite = try $ do
  char '['
  spnl
  citations <- citeList
...
citeKey :: PMonad m => MarkdownParser m (Bool, String)
citeKey = try $ do
...
suffix :: PMonad m => MarkdownParser m (F Inlines)
suffix = try $ do
...
prefix :: PMonad m => MarkdownParser m (F Inlines)
prefix = trimInlinesF . mconcat <$>
  manyTill inline (char ']' <|> liftM (const ']') (lookAhead citeKey))

citeList :: PMonad m => MarkdownParser m (F [Citation])
citeList = fmap sequence $ sepBy1 citation (try $ char ';' >> spnl)

citation :: PMonad m => MarkdownParser m (F Citation)
citation = try $ do
  pref <- prefix
...
smart :: PMonad m => MarkdownParser m (F Inlines)
smart = do
  getOption readerSmart >>= guard
  doubleQuoted <|> singleQuoted <|>
    choice (map (return . B.singleton <$>) [apostrophe, dash, ellipses])

singleQuoted :: PMonad m => MarkdownParser m (F Inlines)
singleQuoted = try $ do
  singleQuoteStart
...
doubleQuoted :: PMonad m => MarkdownParser m (F Inlines)
doubleQuoted = try $ do
  doubleQuoteStart
